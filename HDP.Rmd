...
library(caret)
registerDoSNOW(8)
in_train <- createDataPartition(combine_data$Label, p = 0.8, list = FALSE)
train <- combine_data[in_train, ]
test <- combine_data[-in_train, ]
trControl <- trainControl(method = "cv", number = 5, savePredictions = TRUE)
set.seed(200)
model <- train(Label~., data = train, type = "classification", method = "rf", trControl = trControl, predict.all = TRUE)
conform <- ConformalClassification$new()
conform$Calculate$CVScores(model = model)
conform$CalculatePValues(new.data = test)
conform$C
